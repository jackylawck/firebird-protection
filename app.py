...
# ----------------- GitHub Key 相容 API 呼叫函式 -----------------
def call_github_copilot_ai(token, messages, max_tokens=800):
    clean_token = token.strip() if token else ""
    
    # 切換至 GitHub Copilot 服務端點
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return f"❌ 通訊連線失敗 ({response.status_code})，請確認 GitHub Key 權限！"
    except Exception as e:
        return f"❌ 火鷹俠對講機訊號微弱，請稍後再試！({str(e)})"
...
if not github_token:
    st.warning("👈 請爸爸先在 Streamlit Secrets 或左邊「家長設定區」設定 GitHub Token，幫火鷹俠對講機充電喔！")
else:
    tab1, tab2 = st.tabs(["📖 互動故事冒險", "📻 火鷹俠全能學習對講機"])

    # 🛡️ 核心教育精神 System Prompt
        ...
        if len(st.session_state.ai_history) == 0:
            if st.button("🚀 點擊開始全新的火鷹俠冒險！"):
                ...
                with st.spinner("🚀 火鷹俠正在登場中..."):
                    reply = call_github_copilot_ai(github_token, messages, max_tokens=800)
            ...
            if st.button("🔥 確定！翻去下一頁！ (Next Page!)"):
                if user_choice:
                    ...
                    with st.spinner("🚀 火鷹俠正在飛往下一個場景..."):
                        reply = call_github_copilot_ai(github_token, api_messages, max_tokens=800)
                        st.session_state.ai_history.append({"role": "assistant", "content": reply})
                        st.rerun()
        ...
        if prompt := st.chat_input("📻 對火鷹俠說話... (例如：恐龍點解會絕種？)"):
            ...
            with st.chat_message("assistant", avatar="📻"):
                with st.spinner("📻 火鷹俠正在思考並按對講機回覆你..."):
                    chat_reply = call_github_copilot_ai(github_token, api_chat_messages, max_tokens=500)
